...
function getTrackList() {
	...
	var fav = ' <a href="#"><i class="icon-star-empty fav"></i></a>';
	$.getJSON(url, function(data){
		for (var i=0; i<data.length; i++) {
			if ( data[i].type == "track" ) {
				$("#bar").css('width','0px')
				$("#currentSong").html(data[i].track + fav);
				if ( air.EncryptedLocalStore.getItem('tracklisting') == 'true' ) {
					notifyTrack(data[i].track);
				}
				$('.fav').click(function() {
					$(this).removeClass("icon-star-empty").addClass("icon-star");
					addFav(data[i].artist, data[i].title);
				});
				var current = Math.round(+new Date()/1000);
				var reset = data[i].started + data[i].duration + 10 - current;
				var prtDone = current - data[i].started;
				var prtDonePer = Math.round(prtDone / data[i].duration * 100);
				var perPer = Math.round(prtDone / prtDonePer * 1000);
				air.trace(perPer);
				$("#bar").css('width', prtDonePer + '%');
				
				progressbar = setInterval(function() {
					var percent = $("#bar").css('width').replace('px','');
					air.trace(percent);
					percent = percent / 3;
					percent = percent + 1;
					$("#bar").css('width', percent + '%');
					air.trace(percent + '% of the song played so far.')
					if ( percent == 100 ) {
						clearInterval(progressbar);
					}
				}, perPer);
				...
				TRACKTIMER = setTimeout(function() {
					getTrackList();
					//air.trace(current + ' ' + reset + ' ' + data[i].started + ' ' + data[i].duration)
				}, reset); 
				break;
			}
		};
